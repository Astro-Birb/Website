...
const DropdownSearch: React.FC<DropdownSearchProps> = ({ label, roles, selectedRoles, setSelectedRoles }) => {
    ...
    const handleSelectRole = (role) => {
        ...
        if (selectedRoles.some(selectedRole => selectedRole.id === role.id)) {
            //@ts-ignore
            setSelectedRoles(selectedRoles.filter(selectedRole => selectedRole.id !== role.id));
        } else {
                    ...
                    <div className="flex flex-wrap">
                        ...
                        {selectedRoles.map(role => (
                            <span key={role.id} className="flex items-center m-1 p-1 bg-zinc-600 text-white text-sm rounded">
                                {role.name}
                                <button onClick={() => handleSelectRole(role)} className="ml-1 text-white">
                                    &times;
                                </button>
                            </span>
                        ))}
                        ...
                        {filteredRoles.map((role) => (
                            <li key={role.id}>
                                <label className="flex items-center pl-2 rounded hover:bg-gray-100 dark:hover:bg-zinc-700 cursor-pointer">
                                    ...
                                    />
                                    <span className="w-full py-2 ml-2 text-sm font-medium text-zinc-900 rounded dark:text-gray-300">{role.name}</span>
                                </label>
                            </li>
                        ))}
                    </ul>
